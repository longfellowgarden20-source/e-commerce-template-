        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-wrap gap-2">
          {filters.map((f) => (
            <button
              ...
              className={`px-4 py-2 rounded-full text-sm font-medium border cursor-pointer transition-colors ${f === activeFilter ? 'bg-accent text-white border-accent' : 'bg-white text-slate-600 border-slate-200 hover:border-accent hover:text-accent'}`}
            >
              {f}
            </button>
          ))}
